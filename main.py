...
class MenuPlugin(Star):
    def __init__(self, context: Context):
        ...
        # 从配置中读取查询地址
        # 配置路径: data/config/astrbot_plugin_menu.yaml
        self.query_url = self.config.get("query_url", "")
    ...
    async def query_progress(self, event: AstrMessageEvent, phone: str = ""):
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(self.query_url, params=params, data=data, timeout=30) as resp:
                    if resp.status != 200:
